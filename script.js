...
const getWeatherData = async () => {
  ...
  try {
    // create a const that stores userInput from the searchbar or defaults back to 'Los Angeles' if left blank
    const city = searchInput.value || 'Los Angeles'

    // Create 2 promises that call the APIs and pass in the city name
    // If the user hasn't typed in anything, use Los Angeles as default
    const currentWeather = new Promise(async (resilve, reject) => {

      try {
        const weatherApiData = await fetch(  `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=8109965e7254a469d08a746e8b210e1e&units=imperial`,)
      }
...
const forecast = new Promise(async (resolve, reject) => {
  try {
    const forecastApiData = await fetch(       `https://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=8109965e7254a469d08a746e8b210e1e&units=imperial&cnt=10`,
    )
...
// Now pass that data into the upsideDown() function
updateDom(data)
  }
catch (error) {
  console.log(error)
...
// Hint: Draw a circle and visualize each direction first.
...
// Update the DOM element with the API data
const updateDom = data => {
  ...
  // Weather Icon
  // Use the template literals to set in the  link, then set it as image source:
  ...
  // Wind Direction (Using the getDirection function)
  ...
  const sunriseTs = new Date
  (data[0].sys.sunrise *1000)
